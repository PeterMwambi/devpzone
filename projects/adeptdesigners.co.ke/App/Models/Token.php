...
class Token
{
    ...
    private static function generate(string $tokenName)
    {
        if (!empty($tokenName)) {
            ...
            $encryptedToken = Functions::encrypt(self::$token);
    ...
    public function  showContents()
    {
        if (isset(self::$tokenName)) {
            return Functions::decrypt(Session::getValue(self::$tokenName));
        }
    }
    ...
    private static function processStorage()
    {
        if (isset(self::$tokenName) && isset(self::$token)) {
            Session::generate(self::$tokenName, Functions::encrypt(self::$token), "array");
            return true;
        }
        return false;
    }
    ...
    {
        if (!empty($tokenName)) {
            $tokenName = Functions::decrypt($tokenName);
            die($tokenName);
            if (Session::exists($tokenName)) {
                $known_token = Functions::decrypt(Session::getValue($tokenName));
                $user_token = Functions::decrypt($token_value);
