...
class File
{
    ...
    private function processWrite(string $filename, string $uploadDirectory)
    {
        if (!empty($filename) && !empty($uploadDirectory)) {
            $this->filename = Sanitize::__string($_FILES[$filename]["name"]);
            $this->tmp_filename = Sanitize::__string($_FILES[$filename]["tmp_name"]);
            $this->filesize = Sanitize::__string($_FILES[$filename]["size"]);
