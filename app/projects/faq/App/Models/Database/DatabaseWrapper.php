...
use Models\Core\Config\Settings as Config;
...
class DatabaseWrapper
{
    ...
    protected function runQuery($sql, $params = array(), $fetch = 0, $fetchOptions = null)
    {
        if ($sql !== null) {
            if (!$this->executeQuery($sql, $params, $fetch, $fetchOptions)->error()) {
                return $this;
            }
            return false;
        }
    ...
    private function executeQuery($sql, $params = array(), $fetch = 0, $fetchOptions = null)
    {
        $this->errors = false;
        ...
        if ($this->stmt->execute()) {
            ...
            if ($fetch === 0) {
                $this->results = $this->stmt->fetchAll($fetchOptions);
            }
            if ($fetch === 1) {
     ...
     * }
     * if(count === 6){
     *          array("INNER JOIN", "orders", "ON", "customer.customer_id", "=", "orders.customer_id", "where", "user_id" "=" Session::get("user_id"))
     * }
     * if (count === 8) {
    ...
    protected function generateSelectQuery(
        $action_fields = array(), $table, $fetch = 0, $where = array())
    {
